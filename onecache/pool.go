...
func (c *conn) getClient() (*streamClient, error) {
	...
	codec := msgpackrpc.NewCodec(false, false, stream)
...
// Acquire is used to get a connection that is pooled or to return a new
// connection
...
func (p *connPool) getClient(addr net.Addr) (*conn, *streamClient, error) {
	retries := 0
START:
